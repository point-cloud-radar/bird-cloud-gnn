...
def generate_data(
    ...
    max_range=300_000.0,
    azimuth_skip=2.0,
    elevations=np.array([0.3, 0.8, 1.2, 2, 2.8, 4.5, 6, 8, 10, 12, 15, 20,
                         25]),
    add_na=False,
    z_position=343.0,
    ...
    point_cloud = pd.DataFrame({
        'range':
        min_range + np.exp(-5 * np.random.rand(num_points)) * (max_range - min_range),
        'azimuth':
        np.tile(
            np.arange(0, 360, azimuth_skip) + 0.5,
            ceil(num_points / 360 * azimuth_skip))[0:num_points],
        'elevation':
        sorted(
            np.tile(elevations,
                    ceil(num_points / len(elevations)))[0:num_points]),
        'useless_feature':
        np.random.randn(num_points),
    })

    c_elevation, s_elevation = np.cos(
        np.pi / 180 * point_cloud.elevation), np.sin(np.pi / 180 *
    ...
    if add_na:
        point_cloud.loc[np.random.randint(0, num_points, num_points // 100),
                        'feat2'] = None
        point_cloud.loc[np.random.randint(0, num_points, num_points // 20),
                        'feat3'] = None
